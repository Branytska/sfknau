<?php
    ...
    $query = "SELECT teachers.teacher_name from teachers where teachers.id = '".$teacher."'";
    ...
    $pdf->Cell(450,20,iconv('utf-8', 'windows-1251',"Звіт по викладачу "),0,0,'C');
    $pdf->Ln();
    $pdf->Cell(450,20,iconv('utf-8', 'windows-1251',$row['teacher_name']),0,0,'C');
    $pdf->Ln();
    $pdf->Cell(450,20,iconv('utf-8', 'windows-1251',"за період з ".$date_from." по ".$date_to),0,0, 'C');
    $pdf->Ln();
    $pdf->SetFont('TimesNewRomanPSMT','', 14);
    $pdf->Cell(100,30,iconv('utf-8', 'windows-1251',"Група"),1,0,'C');
    ...
    $query = "select study_groups.group_name, subjects.subject_name, types_of_activities.activity_name, sum(main_activities.numbers_of_hours)
from subjects, main_activities, study_groups, types_of_activities
where main_activities.subject_id = subjects.id 
    and main_activities.activity_id = types_of_activities.id
    and main_activities.group_id = study_groups.id 
    and main_activities.teacher_id = ".$teacher."
        ...
        where main_activities.subject_id = subjects.id 
            and main_activities.activity_id = types_of_activities.id
            and main_activities.group_id = study_groups.id 
            and main_activities.teacher_id = ".$teacher." and study_groups.group_name in (
                select study_groups.group_name 
                from study_groups, main_activities 
                    where main_activities.teacher_id = ".$teacher." and main_activities.group_id = study_groups.id 
                        and main_activities.date_activity between '".$date_from."' and '".$date_to."'
                    group by study_groups.group_name
            )
    )
    and study_groups.group_name in (
        select study_groups.group_name 
        from study_groups, main_activities 
        where main_activities.teacher_id = ".$teacher." 
            and main_activities.group_id = study_groups.id 
            and main_activities.date_activity between '".$date_from."' and '".$date_to."'
        group by study_groups.group_name) 
group by study_groups.group_name, subjects.subject_name, types_of_activities.activity_name
";
    $result = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link));
    ...
    if($result){    
        ...
        while ($row = mysqli_fetch_array($result)) {
            $pdf->Cell(100,30,iconv('utf-8', 'windows-1251',$row['group_name']),'LR',0,'L',0);
